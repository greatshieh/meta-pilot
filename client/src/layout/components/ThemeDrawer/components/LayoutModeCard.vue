<template>
    <div class="flex items-center justify-center flex-wrap gap-x-32px gap-y-16px">
        <div
            v-for="(item, key) in layoutConfig"
            :key="key"
            class="flex flex-col cursor-pointer border-2px rounded-6px hover:border-primary"
            ...
            <ElTooltip :placement="item.placement">
                ...
                <div
                    ...
                    :class="[key.includes('vertical') ? 'flex' : 'flex flex-col']">
                    <slot :name="key"></slot>
                </div>
            </ElTooltip>
        </div>
