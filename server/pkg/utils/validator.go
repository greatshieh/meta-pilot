...
// 对非GET请求的参数绑定和验证
func VerifyBindJson[T any](c *gin.Context, obj *T, validator ...Rules) error {
	if err := c.ShouldBindJSON(&obj); err != nil {
		return errors.WithCode(errcode.ErrBind, "%s", err.Error())
	}

	if len(validator) > 0 {
		rObj := reflect.ValueOf(obj)
		o := rObj.Elem().Interface()
		if rObj.Kind() == reflect.Pointer {
			for _, v := range validator {
				if err := Verify(o, v); err != nil {
					return errors.WithCode(errcode.ErrValidation, err.Error())
				}
			}
...
// 对GET请求包含query参数绑定和验证
func VerifyBindQuery[T any](c *gin.Context, obj *T, validator ...Rules) error {
	if err := c.ShouldBindQuery(obj); err != nil {
		return errors.WithCode(errcode.ErrBind, err.Error())
	}
